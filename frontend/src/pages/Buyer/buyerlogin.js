...
import './buyer.css'
import {BuyerLogin,BuyerSignup} from "../../api"
...
const BuyerAuthPage = () => {
    ...
    const handleLoginSubmit = async (e) => {
        ...
        try {
            const response = await BuyerLogin({email:email,password:password});
        console.log(response);//Buyerlogin
        localStorage.setItem("Buyerlogin",true)
        localStorage.setItem("Sellerlogin",false)
        localStorage.setItem("BuyerID",response.buyer._id)
    ...
    const handleSignupSubmit = async(e) => {
        ...
        try {
            const response = await BuyerSignup({ first_name: first_name,last_name:last_name, email:email1 , password: password1,mobile:Mobile});
